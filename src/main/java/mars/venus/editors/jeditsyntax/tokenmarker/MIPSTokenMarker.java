/*
 * MIPSTokenMarker.java - MIPS Assembly token marker Copyright (C) 1998, 1999
 * Slava Pestov, 2010 Pete Sanderson
 *
...
public class MIPSTokenMarker extends TokenMarker {
	...
	public ArrayList getTokenExactMatchHelp(final Token token, final String tokenText) {
		...
		if (token != null && token.id == Token.KEYWORD1) {
			final ArrayList instrMatches = mars.Globals.instructionSet.matchOperator(tokenText);
	...
	public ArrayList getTokenPrefixMatchHelp(final String line, final Token tokenList, final Token token, final String tokenText) {
		...
		if (token != null && token.id == Token.COMMENT1) { return null; }
		...
		if (keywordTokenText != null) {
			if (keywordType == Token.KEYWORD1) { return getTextFromInstructionMatch(keywordTokenText, true); }
			if (keywordType == Token.KEYWORD2) { return getTextFromDirectiveMatch(keywordTokenText, true); }
		}
		...
		if (token != null && token.id == Token.NULL) {
			...
			if (keywordTokenText == null && trimmedTokenText.length() == 0) { return null; }
